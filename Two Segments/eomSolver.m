...
function dqdt = eomSolver(t,yy,node1,l,h,m,g,F1,F2,F3,F4,F5)
%INPUT: yy = 12 states
%OUTPUT:
    x = yy(1); y = yy(2); z = yy(3); theta = yy(4); phi = yy(5); psi = yy(6);
    xd = yy(7); yd = yy(8); zd = yy(9); thetad = yy(10); phid = yy(11); psid = yy(12);
    dqdt = zeros(12,1);
    F = appliedForce(node1,l,h,m,g,F1,F2,F3,F4,F5,x,y,z,theta,phi,psi);
    
    dqdt(1:6) = [xd,yd,zd,thetad,phid,psid]';
    dqdt(7:12) = [F(1)/(5*m),F(2)/(5*m),F(3)/(5*m),...
                (F(4)-F(6)*sind(phi)-4*m*cosd(phi)*phid*((l^2)*psid+2*(-h^2+l^2)*sind(phi)*thetad))/...
                (m*(2*(h^2)+3*(l^2)+(2*(h^2)-(l^2))*cosd(2*phi)-4*(l^2)*((sind(phi))^2))),...
